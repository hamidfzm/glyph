...
- All color values use CSS custom properties (`var(--color-*)`) for theme support. The palette, accent and reading face are defined in [docs/brand.md](../../docs/brand.md), which also lists the values that carry the accent as a separate literal and drift if changed alone; read it before editing `--color-accent` or the reading stack.
