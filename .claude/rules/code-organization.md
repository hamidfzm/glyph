...
- **Core plugins are the exception**: each lives whole in `src/plugins/core/<id>/` and imports only its own folder, the public plugin types (`@/lib/plugins/types`), and npm packages, never the app's React, hooks, i18n, or `src/lib` helpers. It reaches the app through `ctx` alone, like a community plugin, so any capability it needs is added to the public plugin API first.
