 ...
 * `.ipynb` JSON is worse than nothing. Source documents (a `.d2` diagram, a
 * plugin file type) are one block of source (D2's `#` comments would otherwise
 * read as headings), so they suppress too: no word count, read-aloud, AI text,
