...
const t = ((key: string) => key) as unknown as Parameters<typeof buildTabMenuItems>[4];
...
describe("buildTabMenuItems", () => {
  ...
  it("returns nothing when the target tab is gone", () => {
    expect(buildTabMenuItems(tabs(2), "missing", vi.fn(), vi.fn(), t)).toEqual([]);
  });

  it("omits Close to the Left on the first tab", () => {
    const items = labels(buildTabMenuItems(tabs(3), "tab-0", vi.fn(), vi.fn(), t));
  ...
  it("omits Close to the Right on the last tab", () => {
    const items = labels(buildTabMenuItems(tabs(3), "tab-2", vi.fn(), vi.fn(), t));
  ...
  it("offers only Close when a single tab is open", () => {
    const items = labels(buildTabMenuItems(tabs(1), "tab-0", vi.fn(), vi.fn(), t));
    expect(items).toEqual([
      "tabBar.contextMenu.close",
      "fileTree.openInNewWindow",
      "fileTree.copyAbsolutePath",
      "fileTree.reveal",
    ]);
  });

  it("opens the target tab's file in a new window", () => {
    const openInNewWindow = vi.fn();
    const items = buildTabMenuItems(tabs(2), "tab-1", vi.fn(), openInNewWindow, t);

    select(items, "fileTree.openInNewWindow");
    expect(openInNewWindow).toHaveBeenCalledWith("/p/file1.md");
  });

  it("closes the target, the others, and each side with the right ids", () => {
    const closeTabs = vi.fn();
    const items = buildTabMenuItems(tabs(4), "tab-1", closeTabs, vi.fn(), t);
  ...
  it("copies the target tab's path", async () => {
    ...
    select(buildTabMenuItems(tabs(2), "tab-1", vi.fn(), vi.fn(), t), "fileTree.copyAbsolutePath");
    expect(writeText).toHaveBeenCalledWith("/p/file1.md");
    vi.unstubAllGlobals();
  });

  it("reveals the target tab's file in the OS file manager", () => {
    select(buildTabMenuItems(tabs(1), "tab-0", vi.fn(), vi.fn(), t), "fileTree.reveal");
  ...
  it("hides the path items for an unsaved document", () => {
    const items = labels(buildTabMenuItems([fileTab(0, true)], "tab-0", vi.fn(), vi.fn(), t));
    expect(items).toEqual(["tabBar.contextMenu.close"]);
  });

  it("hides the path items for a graph tab, which has no single file", () => {
    const items = labels(
      buildTabMenuItems([fileTab(0), graphTab(1)], "tab-1", vi.fn(), vi.fn(), t),
    );
