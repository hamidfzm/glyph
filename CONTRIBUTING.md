...
Glyph also builds as an Android app (see issue #79; single-file viewing only,
desktop features like folder workspaces, sync, and printing are gated off).

Prerequisites: JDK 17+, the Android SDK with NDK (set `ANDROID_HOME`,
`NDK_HOME`, and `JAVA_HOME`), and the Rust targets:
...
The Android project under `src-tauri/gen/android/` is generated by
`pnpm tauri android init` and committed; regenerate it only when the Tauri CLI
requires it. iOS is not wired up yet (it needs a Mac to run `tauri ios init`).
