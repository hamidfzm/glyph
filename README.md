...
- Plugins run sandboxed by default: isolated from the app with file and network access limited to the permissions they declared and you accepted. A plugin that opts out of the sandbox needs a separate, explicit full-access grant (with its own warning), updates that request new permissions ask again, and marketplace downloads are checksum-verified before install. Plugins installed before sandboxing became the default stay off until you re-enable them once through that same consent
