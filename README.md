...
- Export an entire workspace as a static website via `File → Export → Website…` (folder workspaces): one linked HTML page per markdown file with shared styles, cross-page navigation, a generated index (or the root README), resolved wikilinks and relative links, copied images, and inline Mermaid diagrams. Also available headless from the CLI (`glyph <folder> --export-website <outDir>`) for CI publishing, e.g. to GitHub Pages
