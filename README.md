...
- Export an entire workspace as a static website via `File → Export → Website…` (folder workspaces): one linked HTML page per markdown file with shared styles, cross-page navigation, a per-page outline, an index built from the root index.md (falling back to the root README, then a generated page list), resolved wikilinks and relative links, copied images, and inline Mermaid diagrams. Also available headless from the CLI (`glyph <folder> --export-website <outDir>`) for CI publishing, e.g. to GitHub Pages
