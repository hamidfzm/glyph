...
- Canvas — open [JSON Canvas](https://jsoncanvas.org) (`.canvas`) files as an infinite, pan-and-zoom board; cards render markdown, embedded images, links, and labelled groups, connected by arrows. View mode is read-only; switch to edit mode to move, resize, recolour, and connect cards, edit text inline, and add or delete nodes and edges. Edits save back as standard `.canvas` JSON that round-trips with Obsidian, with undo/redo per tab
