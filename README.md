...
- Images: image and SVG files (`.png`, `.jpg`, `.gif`, `.webp`, `.bmp`, `.svg`, and more) in a workspace folder show in the sidebar tree; click one to open it in a read-only viewer with fit, actual-size, and zoom controls, `Cmd/Ctrl+=/-/0` or `Cmd/Ctrl`+scroll to zoom, and drag or scroll to pan a zoomed image
