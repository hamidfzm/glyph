...
Canvas boards integrate with the rest of the app: `File → Export` saves the whole board as a PNG image, the status bar word count (and AI / read-aloud) reads the board's cards rather than its JSON, and canvases sync and back up like any other workspace file.
